    ...
    for (; r > 0 || i < dp.parts_n; r--) {
        ...
        switch (part.m) {
            ...
            case DESYNC_FAKE:;
                ...
                if (LOG_ENABLED) {
                    char *fdata = pkt.data + pkt.off;
                    ssize_t fsz = pkt.size - pkt.off;
                    char *fsni = 0;
                    int fsni_len = parse_tls(fdata, fsz, &fsni);
                    if (fsni_len > 0) {
                        LOG(LOG_S, "fake_sni: %.*s (%d), off=%zd, pos=%ld-%ld\n",
                            fsni_len, fsni, fsni_len, pkt.off, lp, pos);
                    } else {
                        ssize_t hex_len = fsz > 64 ? 64 : fsz;
                        INIT_HEX_STR(fdata, hex_len);
                        LOG(LOG_S, "fake_hex: %s, off=%zd, pos=%ld-%ld\n",
                            HEX_STR, pkt.off, lp, pos);
                    }
                }
