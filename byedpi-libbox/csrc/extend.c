        ...
        struct eval *val, struct buffer *buff)
{
    ...
    if (val->flag == FLAG_CONN) {
        ...
        if (val->pair->sq_buff) {
            ...
            if (is_tls_shello(buff->data, n)) {
                LOG(LOG_S, "success: fd=%d, tls_resp=%zd bytes\n", val->fd, n);
            } else {
                INIT_HEX_STR(buff->data, (n > 16 ? 16 : n));
                LOG(LOG_S, "response: fd=%d, n=%zd, hex=%s\n", val->fd, n, HEX_STR);
            }
...
static int protect(int conn_fd, const char *path)
{
    ...
    if (path[0] == '/') {
        ...
        if (path_len >= sizeof(sa.sun_path)) {
            errno = ENAMETOOLONG;
            uniperror("protect path too long");
            return -1;
        }
        sa.sun_path[0] = '\0';
        memcpy(sa.sun_path + 1, path, path_len);
        sa_len = (socklen_t)(offsetof(struct sockaddr_un, sun_path) + 1 + path_len);
    }
