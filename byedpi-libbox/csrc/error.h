...
static int unie(int e)
{
    ...
    #define LOG_S 1
    #define LOG_L 2
    static inline int byedpi_log_level(int level) {
        switch (level) {
            case LOG_E: return ANDROID_LOG_ERROR;
            case LOG_S: return ANDROID_LOG_INFO;
            case LOG_L: return ANDROID_LOG_DEBUG;
            default:    return ANDROID_LOG_DEBUG;
        }
    }
    #define LOG(s, str, ...) do { \
        if (params.debug >= s) { \
            __android_log_print(byedpi_log_level(s), "proxy", str, ##__VA_ARGS__); \
        } \
    } while(0)
