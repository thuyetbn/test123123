        ...
        struct eval *val, const union sockaddr_u *dst, evcb_t next)
{
    ...
    /* TCP keep-alive: detect dead connections quickly on Android
     * where the default ~2h idle timeout causes zombie sockets. */
#ifdef __linux__
    /* Enable TCP keep-alive probes */
    if (setsockopt(sfd, SOL_SOCKET, SO_KEEPALIVE,
            (char *)&one, sizeof(one))) {
        uniperror("setsockopt SO_KEEPALIVE");
    }
    /* Seconds of idle before sending the first probe (default: 7200s) */
    int idle = 30;
    setsockopt(sfd, IPPROTO_TCP, TCP_KEEPIDLE,
            (char *)&idle, sizeof(idle));
    /* Seconds between successive probes when no ACK received */
    int intvl = 10;
    setsockopt(sfd, IPPROTO_TCP, TCP_KEEPINTVL,
            (char *)&intvl, sizeof(intvl));
    /* Number of unacknowledged probes before declaring the connection dead */
    int cnt = 6;
    setsockopt(sfd, IPPROTO_TCP, TCP_KEEPCNT,
            (char *)&cnt, sizeof(cnt));
    /* Milliseconds: max time for unacknowledged data before aborting.
     * Only set when no user-configured timeout (--timeout flag). */
#ifdef TCP_USER_TIMEOUT
    int ut = 120000;
    if (!params.timeout) {
        setsockopt(sfd, IPPROTO_TCP, TCP_USER_TIMEOUT,
                (char *)&ut, sizeof(ut));
    }
#endif
#endif
...
#ifdef __linux__
        /* Enable TCP keep-alive probes */
        if (setsockopt(c, SOL_SOCKET, SO_KEEPALIVE,
                (char *)&one, sizeof(one))) {
            uniperror("setsockopt SO_KEEPALIVE");
        }
        /* Seconds of idle before sending the first probe (default: 7200s) */
        int idle = 30;
        if (setsockopt(c, IPPROTO_TCP, TCP_KEEPIDLE,
                (char *)&idle, sizeof(idle))) {
            uniperror("setsockopt TCP_KEEPIDLE");
        }
        /* Seconds between successive probes when no ACK received */
        int intvl = 10;
        if (setsockopt(c, IPPROTO_TCP, TCP_KEEPINTVL,
                (char *)&intvl, sizeof(intvl))) {
            uniperror("setsockopt TCP_KEEPINTVL");
        }
        /* Number of unacknowledged probes before declaring the connection dead */
        int cnt = 6;
        if (setsockopt(c, IPPROTO_TCP, TCP_KEEPCNT,
                (char *)&cnt, sizeof(cnt))) {
            uniperror("setsockopt TCP_KEEPCNT");
        }
        /* Milliseconds: max time for unacknowledged data before aborting.
         * Only set when no user-configured timeout (--timeout flag). */
#ifdef TCP_USER_TIMEOUT
